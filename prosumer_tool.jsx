...
const SUB_PROFILES = {
  ...
  warmwasser: {
    winter:    [0.015,0.012,0.010,0.010,0.012,0.025,0.060,0.080,0.065,0.045,0.035,0.035,0.040,0.038,0.035,0.038,0.045,0.060,0.075,0.070,0.060,0.045,0.030,0.020],
    uebergang: [0.014,0.011,0.009,0.009,0.011,0.024,0.058,0.078,0.063,0.043,0.033,0.033,0.038,0.035,0.032,0.035,0.043,0.058,0.072,0.068,0.058,0.043,0.028,0.019],
    sommer:    [0.012,0.010,0.008,0.008,0.010,0.022,0.055,0.075,0.060,0.040,0.030,0.030,0.035,0.032,0.030,0.032,0.040,0.055,0.070,0.065,0.055,0.040,0.025,0.018],
  },
  ev: {
...
function evaluateConfig(load, totalLoad, kwp, batteryKWh, params, pvShape) {
  ...
  const stromkostenMitPV = totalImport * params.strompreis - totalExport * params.einspeiseverguetung;
...
const STORAGE_KEY = 'prosumer_tool_state_v6';
...
export default function ProsumerTool() {
  ...
  const sim = useMemo(() => {
    ...
    const stromkostenMitPV = totalImport * state.strompreis - totalExport * state.einspeiseverguetung;
    const ersparnis = stromkostenOhnePV - stromkostenMitPV;
    const jahreskostenPV = pvAnnuity + speicherAnnuity + betriebskosten;
    const nettoErsparnis = ersparnis - jahreskostenPV;
    ...
    const daily = aggregateDaily({load, pv, gridImport: result.gridImport, gridExport: result.gridExport, soc: result.soc, batteryCharge: result.batteryCharge, batteryDischarge: result.batteryDischarge});
    ...
    return {
      ...
      heatmapLoad, heatmapPV, heatmapImport,
    };
  }, [state]);
  ...
  if (!loaded) {
    return <div style={{padding: 40, fontFamily: 'monospace', background: '#0a0e14', color: '#9ca3af', minHeight: '100vh'}}>Lade gespeicherten Zustand…</div>;
  }
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={gridStyle}>
        ...
        <div style={styles.card}>
          <div style={styles.cardLabel}>Jaehrliche Lastkurve</div>
          <ResponsiveContainer width="100%" height={isMobile ? 200 : 260}>
            <AreaChart data={sim.daily} margin={{top:10,right:10,left:-20,bottom:0}}>
              ...
              <Tooltip contentStyle={{background:'#0f1419', border:'1px solid #1f2937', fontSize:12}} />
              <Area type="monotone" dataKey="load" stroke="#f59e0b" fill="#f59e0b22" name="Last [kWh/d]" />
            </AreaChart>
          </ResponsiveContainer>
    ...
    <div style={styles.panel}>
      ...
      <div style={gridStyle}>
        <div style={styles.card}>
          ...
          <Slider label="Strompreis (Bezug)" value={state.strompreis} min={0.15} max={0.60} step={0.005} unit="EUR/kWh" decimals={3} onChange={v=>update('strompreis',v)} hint="Was du heute pro kWh aus dem Netz bezahlst. Mittelwert DE 2025: ~0.32 €/kWh" />
          ...
          <Slider label="PV-Kosten" value={state.pvKostenProKWp} min={800} max={2500} step={50} unit="EUR/kWp" onChange={v=>update('pvKostenProKWp',v)} hint="Schlüsselfertig EFH: 1200-1600 €/kWp inkl. Wechselrichter und Montage" />
    ...
    <div style={styles.panel}>
      ...
      <div style={gridStyle}>
        ...
        <div style={styles.card}>
          ...
          <div style={{fontSize:10, color:'#4b5563', marginTop:14, fontFamily:'JetBrains Mono, monospace', lineHeight:1.6}}>
            Die Optimierung berechnet alle möglichen Anlagengrößen im Rahmen und liefert <strong style={{color:'#9ca3af'}}>sieben verschiedene Empfehlungen</strong>. Jede beantwortet die Frage "was ist die beste Größe?" auf eine andere Art.
          </div>
        </div>
        ...
        <div style={styles.card}>
          {!optResult && !optimizing && (
            <div style={{padding:'40px 20px', textAlign:'center', color:'#6b7280', fontFamily:'JetBrains Mono, monospace', fontSize:12, lineHeight:1.7}}>
          ...
          {optimizing && (
            <div style={{padding:'40px 20px', textAlign:'center', color:'#22d3ee', fontFamily:'JetBrains Mono, monospace', fontSize:12}}>
              <div style={{fontSize:24, marginBottom:12}}>◐</div>
              Berechne Konfigurationen…
            </div>
          )}
          {optResult && !optimizing && (
            <ResultSummary optResult={optResult} state={state} isMobile={isMobile} />
          )}
        </div>
      </div>
      ...
      {optResult && !optimizing && (
        <>
          {/* STRATEGIEN als Cards */}
          <div style={{...styles.card, marginTop:24}}>
            <div style={styles.cardLabel}>Empfehlungen · Sweet-Spot-Methoden</div>
            <div style={{fontSize:11, color:'#9ca3af', marginBottom:14, fontFamily:'JetBrains Mono, monospace', lineHeight:1.5}}>
              Verschiedene Wege, den ausgewogenen Punkt zwischen Investition und Ertrag zu bestimmen. Liegen die Empfehlungen nahe beieinander, ist die Wahl eindeutig.
            </div>
            <StrategyGrid 
              strategies={optResult.strategies.filter(s => s.group === 'pareto')}
              state={state} applyOptimum={applyOptimum} isMobile={isMobile}
          ...
          <div style={{...styles.card, marginTop:24}}>
            ...
            <div style={{fontSize:11, color:'#9ca3af', marginBottom:14, fontFamily:'JetBrains Mono, monospace', lineHeight:1.5}}>
              Die Anlage, die einen einzelnen Aspekt maximiert. Selten die beste Gesamtwahl, aber gut zur Einordnung.
            </div>
            <StrategyGrid 
          ...
          {/* Pareto-Diagramm mit allen Strategiepunkten */}
          <div style={{...styles.card, marginTop:24}}>
              ...
              <ScatterChart margin={{top:10,right:20,left:0,bottom:30}}>
                ...
                <Tooltip cursor={{strokeDasharray:'2 4'}}
                  content={({active, payload}) => {
                    ...
                    return (
                      <div style={{background:'#0f1419', border:'1px solid #1f2937', padding:'8px 10px', fontFamily:'JetBrains Mono, monospace', fontSize:11}}>
                        <div style={{color:'#22d3ee', marginBottom:4}}>{d.kwp} kWp · {d.batteryKWh} kWh</div>
                        <div style={{color:'#9ca3af'}}>Invest: {d.totalInvest.toLocaleString('de-DE',{maximumFractionDigits:0})} €</div>
                        ...
                        <div style={{color:'#9ca3af'}}>ROI: {(d.roi*100).toFixed(2)} %</div>
                        <div style={{color:'#9ca3af'}}>Autarkie: {(d.autarkie*100).toFixed(1)} %</div>
                      </div>
                    );
                  }}
                />
                <Scatter name="Alle" data={optResult.results} fill="#374151" fillOpacity={0.4} />
                <Scatter name="Pareto" data={optResult.pareto} fill="#9ca3af" fillOpacity={0.7} />
                {optResult.strategies.map(s => s.config && (
                  <Scatter key={s.id} name={s.label} data={[s.config]} fill={s.color} shape={s.shape} />
                ))}
              </ScatterChart>
            </ResponsiveContainer>
            <div style={{fontSize:10, color:'#4b5563', marginTop:12, fontFamily:'JetBrains Mono, monospace', display:'flex', gap:'8px 16px', flexWrap:'wrap'}}>
              <span><span style={{color:'#374151'}}>●</span> alle Größen</span>
              <span><span style={{color:'#9ca3af'}}>●</span> beste Größen je Investition</span>
              {optResult.strategies.filter(s=>s.config).map(s => (
                <span key={s.id}><span style={{color: s.color}}>{shapeIcon(s.shape)}</span> {s.label}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{...styles.card, marginTop:24}}>
            ...
            <div style={{fontSize:11, color:'#9ca3af', marginBottom:12, fontFamily:'JetBrains Mono, monospace', lineHeight:1.5}}>
              Wenn die blaue Kurve unter die rote Linie (Vergleichszins) fällt, lohnt sich jeder weitere Euro nicht mehr im Vergleich zu deiner Alternativanlage.
            </div>
                ...
                <Tooltip
                  content={({active, payload}) => {
                    ...
                    return (
                      <div style={{background:'#0f1419', border:'1px solid #1f2937', padding:'8px 10px', fontFamily:'JetBrains Mono, monospace', fontSize:11}}>
                        <div style={{color:'#22d3ee', marginBottom:4}}>{d.kwp} kWp · {d.batteryKWh} kWh</div>
                        <div style={{color:'#9ca3af'}}>+{d.deltaInvest.toFixed(0)} € Mehr-Invest → +{d.deltaCashflow.toFixed(0)} €/a Mehr-Ersparnis</div>
...
function ResultSummary({optResult, state, isMobile}) {
  const validStrategies = optResult.strategies.filter(s => s.config && s.config.totalInvest > 0);
  if (validStrategies.length === 0) {
    return (
      <div style={{padding:'20px 0', color:'#6b7280', fontFamily:'JetBrains Mono, monospace', fontSize:12, lineHeight:1.6}}>
        <div style={{color:'#fbbf24', marginBottom:8}}>⚠ Keine wirtschaftliche Lösung gefunden.</div>
        <div>Mögliche Ursachen:</div>
        <div style={{marginTop:8}}>
          • Vergleichszins zu hoch gewählt<br/>
          • Strompreis-Annahme zu niedrig<br/>
          • Kosten für PV/Speicher zu hoch angesetzt<br/>
          • Suchbereich zu eng
        </div>
      </div>
    );
  ...
  // Spreizung der Sweet-Spot-Empfehlungen berechnen
  const paretoStrategies = optResult.strategies.filter(s => s.group === 'pareto' && s.config && s.config.totalInvest > 0);
  const investments = paretoStrategies.map(s => s.config.totalInvest);
  const minInv = Math.min(...investments);
  const maxInv = Math.max(...investments);
  const spread = maxInv > 0 ? (maxInv - minInv) / maxInv : 0;
  const consensus = spread < 0.2;
  
  return (
    <div>
      <div style={styles.cardLabel}>Übersicht</div>
      
      <div style={{padding:14, background:'#0a0e14', border:`1px solid ${consensus ? '#10b981' : '#fbbf24'}`, marginBottom:14}}>
        <div style={{fontSize:11, color: consensus ? '#10b981' : '#fbbf24', fontFamily:'JetBrains Mono, monospace', letterSpacing:'0.1em', marginBottom:6}}>
          {consensus ? '✓ EINDEUTIGE EMPFEHLUNG' : '⚠ MEHRERE OPTIONEN SINNVOLL'}
        </div>
        <div style={{fontSize:11, color:'#9ca3af', fontFamily:'JetBrains Mono, monospace', lineHeight:1.6}}>
          {consensus
            ? 'Alle Sweet-Spot-Methoden kommen auf eine ähnliche Größe. Du kannst dich auf den Vorschlag verlassen.'
            : `Die Empfehlungen unterscheiden sich um ${(spread*100).toFixed(0)} % in der Investitionssumme. Es gibt keinen eindeutig "richtigen" Punkt – wähle nach persönlicher Vorliebe (klein und sicher vs. groß und renditestark).`}
        </div>
      </div>
      
      <div style={{fontSize:11, color:'#9ca3af', fontFamily:'JetBrains Mono, monospace', lineHeight:1.6}}>
        <div style={{marginBottom:8, color:'#6b7280', textTransform:'uppercase', letterSpacing:'0.1em', fontSize:10}}>Spannweite der Empfehlungen</div>
        <div style={{display:'grid', gridTemplateColumns:'auto 1fr', gap:'4px 12px'}}>
          <span style={{color:'#6b7280'}}>Kleinste:</span>
          <span style={{color:'#d1d5db'}}>{(minInv/1000).toFixed(1)} k€</span>
          <span style={{color:'#6b7280'}}>Größte:</span>
          <span style={{color:'#d1d5db'}}>{(maxInv/1000).toFixed(1)} k€</span>
          <span style={{color:'#6b7280'}}>Unterschied:</span>
          <span style={{color: consensus ? '#10b981' : '#fbbf24'}}>{(spread*100).toFixed(0)} %</span>
        </div>
      </div>
      
      <div style={{fontSize:10, color:'#4b5563', fontFamily:'JetBrains Mono, monospace', marginTop:14, lineHeight:1.5}}>
        Geprüfte Anlagengrößen: {optResult.results.length}<br/>
        Davon effizient: {optResult.pareto.length}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap:10}}>
      {strategies.map(s => {
        ...
        if (!c || !isFinite(c.roi)) {
          return (
            <div key={s.id} style={{padding:12, background:'#0a0e14', border:'1px solid #1f2937', borderLeft:`3px solid ${s.color}`, fontFamily:'JetBrains Mono, monospace', fontSize:11, color:'#6b7280'}}>
        ...
        return (
          <div key={s.id} style={{padding:12, background:'#0a0e14', border:'1px solid #1f2937', borderLeft:`3px solid ${s.color}`}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:8}}>
              <div style={{display:'flex', alignItems:'center', gap:8}}>
                <span style={{color: s.color, fontSize:14}}>{shapeIcon(s.shape)}</span>
                <span style={{color: s.color, fontFamily:'JetBrains Mono, monospace', fontSize:11, letterSpacing:'0.05em', fontWeight:600}}>{s.label}</span>
              </div>
              <button
                onClick={()=>setExpandedId(expanded ? null : s.id)}
                style={{background:'transparent', border:'none', color:'#6b7280', fontFamily:'JetBrains Mono, monospace', fontSize:10, cursor:'pointer', padding:'2px 6px'}}
              >
                {expanded ? '▲' : '?'}
              </button>
            </div>
            
            <div style={{fontSize:10, color:'#9ca3af', fontFamily:'JetBrains Mono, monospace', marginBottom:10, lineHeight:1.4}}>
              {s.desc}
            </div>
            
            {expanded && (
              <div style={{padding:10, background:'#0f1419', border:'1px solid #1f2937', marginBottom:10, fontSize:10, color:'#9ca3af', fontFamily:'JetBrains Mono, monospace', lineHeight:1.6}}>
                {s.explainer}
              </div>
            )}
            
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, fontSize:11, fontFamily:'JetBrains Mono, monospace', marginBottom:10}}>
              <div><span style={{color:'#6b7280'}}>Größe </span><span style={{color:'#d1d5db'}}>{c.kwp} kWp / {c.batteryKWh} kWh</span></div>
              <div><span style={{color:'#6b7280'}}>Investition </span><span style={{color:'#d1d5db'}}>{(c.totalInvest/1000).toFixed(1)} k€</span></div>
              ...
              <div><span style={{color:'#6b7280'}}>Autarkie </span><span style={{color:'#d1d5db'}}>{(c.autarkie*100).toFixed(1)} %</span></div>
              <div><span style={{color:'#6b7280'}}>Amortisation </span><span style={{color:'#d1d5db'}}>{isFinite(c.amortisation) ? `${c.amortisation.toFixed(1)} a` : '—'}</span></div>
            </div>
  ...
  return (
    <div style={{marginBottom: 18}}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:6, gap:8}}>
        <span style={{color:'#9ca3af', fontSize:13, letterSpacing:'0.02em'}}>{label}</span>
        <span style={{color:'#f3f4f6', fontFamily:'JetBrains Mono, monospace', fontSize:13, fontWeight:600, whiteSpace:'nowrap'}}>
      ...
      <div style={{position:'relative', height:24, marginBottom:4}}>
        {/* Track */}
        <div style={{position:'absolute', top:10, left:0, right:0, height:4, background:'#1f2937'}} />
        {/* Active range */}
      ...
      {hint && <div style={{fontSize:10, color:'#4b5563', marginTop:4, fontFamily:'JetBrains Mono, monospace'}}>{hint}</div>}
    </div>
  );
}
...
function KpiSmall({label, value, accent}) {
  return (
    <div style={{padding:8, background:'#0a0e14', border:'1px solid #1f2937'}}>
      <div style={{fontSize:9, color:'#6b7280', letterSpacing:'0.1em', textTransform:'uppercase'}}>{label}</div>
      <div style={{fontSize:13, color: accent || '#f3f4f6', fontFamily:'JetBrains Mono, monospace', fontWeight:600, marginTop:2}}>{value}</div>
    </div>
  );
}
...
function ErgebnisPanel({sim, state, isMobile}) {
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={{...styles.kpiBar, flexWrap: 'wrap', gap: isMobile ? 8 : 12}}>
        <KpiBig label="Autarkie" value={`${(sim.autarkie*100).toFixed(1)} %`} accent="#22d3ee" isMobile={isMobile} />
        <KpiBig label="Eigenverbrauch" value={`${(sim.eigenverbrauchsquote*100).toFixed(1)} %`} accent="#fbbf24" isMobile={isMobile} />
        <KpiBig label="Ersparnis p.a." value={`${sim.nettoErsparnis.toLocaleString('de-DE',{maximumFractionDigits:0})} €`} accent={sim.nettoErsparnis >= 0 ? '#10b981' : '#ef4444'} isMobile={isMobile} />
        <KpiBig label="dein Strompreis" value={`${(sim.lcoeEigenverbrauch*100).toFixed(1)} ct`} accent="#a78bfa" isMobile={isMobile} />
        <KpiBig label="Amortisation" value={isFinite(sim.amortisation) ? `${sim.amortisation.toFixed(1)} a` : '—'} accent="#f472b6" isMobile={isMobile} />
      </div>
          ...
          <ComposedChart data={monthly} margin={{top:10,right:10,left:-20,bottom:0}}>
            ...
            <Tooltip
              content={({active, payload, label}) => {
                ...
                const d = payload[0].payload;
                return (
                  <div style={{background:'#0f1419', border:'1px solid #1f2937', padding:'10px 12px', fontFamily:'JetBrains Mono, monospace', fontSize:11, lineHeight:1.6}}>
                    ...
                    <div style={{borderTop:'1px solid #1f2937', marginTop:4, paddingTop:4, color:'#9ca3af'}}>Verbrauch: {d.load} kWh</div>
      ...
      {/* Heatmaps */}
      <div style={{...styles.card, marginBottom:24}}>
        <div style={styles.cardLabel}>Jahres-Heatmap · Stunde × Tag</div>
        <div style={{fontSize:11, color:'#9ca3af', marginBottom:14, fontFamily:'JetBrains Mono, monospace', lineHeight:1.5}}>
          Jeder Pixel ist ein 5-Tages-Mittel zu einer bestimmten Tagesstunde. Hier siehst du Tag-Nacht-Wechsel, Saisonverläufe und Wettervariabilität.
        </div>
        <div style={{display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16}}>
          <Heatmap data={sim.heatmapLoad}   title="Verbrauch [kW]"   colormap="amber" isMobile={isMobile} />
          <Heatmap data={sim.heatmapPV}     title="PV-Erzeugung [kW]" colormap="yellow" isMobile={isMobile} />
          <Heatmap data={sim.heatmapImport} title="Netzbezug [kW]"   colormap="blue" isMobile={isMobile} />
        </div>
      </div>
...
function Heatmap({data, title, colormap = 'amber', isMobile}) {
  ...
  return (
    <div>
      <div style={{fontSize:10, color:'#9ca3af', fontFamily:'JetBrains Mono, monospace', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6}}>{title}</div>
      <div style={{position:'relative'}}>
        ...
        />
        <div style={{display:'flex', justifyContent:'space-between', fontFamily:'JetBrains Mono, monospace', fontSize:8, color:'#6b7280', marginTop:4}}>
          {monthLabels.map((m,i) => <span key={i}>{m}</span>)}
        </div>
      </div>
      <div style={{display:'flex', alignItems:'center', gap:6, marginTop:6, fontSize:9, color:'#6b7280', fontFamily:'JetBrains Mono, monospace'}}>
        <span>0</span>
...
function colorFor(t, colormap) {
  ...
  const palettes = {
    amber:  [[10,14,20], [251,191,36]],
    yellow: [[10,14,20], [253,224,71]],
    blue:   [[10,14,20], [59,130,246]],
  };
  const p = palettes[colormap] || palettes.amber;
...
function gradientFor(colormap) {
  const palettes = {
    amber:  'linear-gradient(to right, #0a0e14, #fbbf24)',
    yellow: 'linear-gradient(to right, #0a0e14, #fde047)',
    blue:   'linear-gradient(to right, #0a0e14, #3b82f6)',
  };
  return palettes[colormap] || palettes.amber;
}
...
function Slider({label, value, min, max, step, unit, onChange, hint, decimals=0, display}) {
  ...
  return (
    <div style={{marginBottom:18}}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:6, gap:8}}>
        <span style={{color:'#9ca3af', fontSize:13, letterSpacing:'0.02em'}}>{label}</span>
        <span style={{color:'#f3f4f6', fontFamily:'JetBrains Mono, monospace', fontSize:14, fontWeight:600, whiteSpace:'nowrap'}}>
...
function ToggleRow({label, value, onChange}) {
  return (
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18}}>
      <span style={{color:'#f3f4f6', fontSize:14, fontWeight:600, letterSpacing:'0.04em', textTransform:'uppercase'}}>{label}</span>
      <button onClick={()=>onChange(!value)} style={{
        background: value ? '#22d3ee' : '#1f2937', border: 'none',
        color: value ? '#0a0e14' : '#9ca3af', padding: '6px 14px',
  ...
  return (
    <div style={{marginBottom:24}}>
      <div style={{display:'flex', alignItems:'baseline', gap:isMobile ? 10 : 16, flexWrap:'wrap'}}>
        <span style={{fontFamily:'JetBrains Mono, monospace', color:'#22d3ee', fontSize:12, letterSpacing:'0.2em'}}>// {nr}</span>
        <h2 style={{margin:0, fontSize:isMobile ? 18 : 24, color:'#f3f4f6', fontWeight:600, letterSpacing:'-0.02em'}}>{title}</h2>
      </div>
      <div style={{color:'#6b7280', fontSize:12, marginTop:4, marginLeft: isMobile ? 0 : 38, fontFamily:'JetBrains Mono, monospace'}}>{sub}</div>
    </div>
  );
}
...
function Kpi({label, value}) {
  return (
    <div style={{padding:'10px 0'}}>
      ...
      <div style={{fontSize:14, color:'#f3f4f6', fontFamily:'JetBrains Mono, monospace', fontWeight:600, marginTop:2}}>{value}</div>
    </div>
  );
}
...
function KpiBig({label, value, accent, isMobile}) {
  return (
    <div style={{
      ...
      background:'#0f1419', border:'1px solid #1f2937', borderTop:`2px solid ${accent}`
    }}>
      <div style={{fontSize:9, color:'#6b7280', letterSpacing:'0.15em', textTransform:'uppercase'}}>{label}</div>
      <div style={{fontSize: isMobile ? 16 : 22, color:'#f3f4f6', fontFamily:'JetBrains Mono, monospace', fontWeight:700, marginTop:6}}>{value}</div>
    </div>
  );
}
...
function Bilanz({label, value, sub, bold, accent, unit='EUR', sign=true}) {
  ...
  const color = accent ? (value >= 0 ? '#10b981' : '#ef4444') : '#f3f4f6';
  return (
    <div style={{
      display:'flex', justifyContent:'space-between',
      padding: sub ? '4px 0 4px 16px' : '6px 0',
      fontSize: sub ? 12 : 13,
      color: sub ? '#6b7280' : '#9ca3af', gap: 8,
    }}>
      <span style={{fontWeight: bold ? 700 : 400, letterSpacing: bold ? '0.05em' : 'normal', textTransform: bold ? 'uppercase' : 'none'}}>{label}</span>
      <span style={{fontFamily:'JetBrains Mono, monospace', color: bold ? color : (sub ? '#6b7280' : '#d1d5db'), fontWeight: bold ? 700 : 500, whiteSpace:'nowrap'}}>{formatted}</span>
    </div>
  );
}

function aggregateDaily({load, pv, gridImport, gridExport, soc, batteryCharge, batteryDischarge}) {
  const result = [];
  for (let d = 0; d < 365; d++) {
    ...
    result.push({day: d+1, load: l, pv: p, gridImport: gi, gridExport: ge, soc: socEnd, batteryCharge: bc, batteryDischarge: bd});
  }
  return result;
}
...
const styles = {
  container: { minHeight: '100vh', background: '#0a0e14', color: '#f3f4f6', fontFamily: '"Inter", -apple-system, sans-serif' },
  header: { display: 'flex', justifyContent: 'space-between', marginBottom: 28, paddingBottom: 20, borderBottom: '1px solid #1f2937' },
  headerLabel: { fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#22d3ee', letterSpacing: '0.3em', marginBottom: 8 },
  headerTitle: { margin: 0, fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1 },
  headerSub: { color: '#6b7280', fontSize: 12, fontFamily: 'JetBrains Mono, monospace', marginTop: 8, letterSpacing: '0.05em' },
  resetBtn: { background: 'transparent', border: '1px solid #1f2937', color: '#9ca3af', padding: '8px 16px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.15em', cursor: 'pointer', alignSelf: 'flex-start' },
  tabNav: { display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid #1f2937' },
  tabBtn: { background: 'transparent', border: 'none', color: '#6b7280', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, letterSpacing: '0.1em', cursor: 'pointer', borderBottom: '2px solid transparent' },
  ...
  card: { background: '#0f1419', border: '1px solid #1f2937', padding: 20 },
  cardLabel: { fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#6b7280', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 16 },
  totalRow: { display: 'flex', justifyContent: 'space-between', paddingTop: 16, marginTop: 16, borderTop: '1px solid #1f2937', fontSize: 14, color: '#22d3ee', fontFamily: 'JetBrains Mono, monospace', textTransform: 'uppercase', letterSpacing: '0.1em' },
  infoRow: { display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: 12, color: '#9ca3af', fontFamily: 'JetBrains Mono, monospace' },
  ...
  footer: { display: 'flex', justifyContent: 'space-between', marginTop: 32, paddingTop: 16, borderTop: '1px solid #1f2937', fontFamily: 'JetBrains Mono, monospace', color: '#4b5563', letterSpacing: '0.1em', textTransform: 'uppercase' },
  optimizeBtn: { width: '100%', background: '#22d3ee', border: 'none', color: '#0a0e14', padding: '14px 20px', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, letterSpacing: '0.2em', cursor: 'pointer', fontWeight: 700 },
  applyBtn: { background: 'transparent', border: '1px solid', padding: '6px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.15em', cursor: 'pointer', fontWeight: 700 },
};
